...
$sql = "SELECT name, email, password FROM staff WHERE unique_id = '$unique_id'";
...
if ($result) {
    ...
    if (pg_num_rows($result) > 0) {
        ...
        echo json_encode(['success' => true, 'name' => $name, 'email' => $email, 'password' => $password]);
    } else {
        // Send error response if unique_id is not found
        echo json_encode(['success' => false, 'error' => 'Staff data not found']);
    }
} else {
